  ...
  workExperience: [
    {
      ...
      endDate: '至今',
      description: [
        '负责公司核心业务的数据指标体系搭建，梳理 30+ 关键业务指标，建立日/周/月报自动化推送机制，将数据产出效率提升 60%',
        '主导用户行为分析项目，通过漏斗分析与留存模型定位流失节点，输出优化方案推动产品迭代，使新用户次日留存率提升 12%',
        '搭建业务可视化看板（Tableau / Power BI），覆盖 5 大业务线，为管理层提供实时数据决策支持，累计支持 20+ 次关键业务评审',
      ],
    },
    {
      id: '2',
      ...
      endDate: '2023年09月',
      description: [
        '协助完成电商大促活动复盘分析，清洗并分析 50w+ 用户行为数据，输出活动效果评估报告',
        '使用 SQL 提取业务数据，配合 Python 完成用户分群建模，为精细化运营提供数据支撑',
      ],
    },
  ],
  projectExperience: [
    {
      ...
      role: '数据分析师',
      description: [
        '设计用户增长分析框架，构建 AARRR 模型与用户生命周期价值（LTV）模型，量化各阶段转化效率',
        '分析用户转化漏斗，识别关键流失节点，结合 RFM 模型对用户分群，提出针对性召回策略',
        '输出 5 份深度分析报告，推动 8 项产品与运营策略调整，项目期间 DAU 提升 15%',
      ],
    },
    {
      id: '2',
      ...
      role: '项目负责人',
      description: [
        '基于历史销售数据，使用 Python（pandas / scikit-learn）搭建时间序列预测模型，预测准确率达 92%',
        '对比 ARIMA、Prophet、XGBoost 三种算法表现，最终选择 XGBoost 并优化特征工程，MAE 降低 18%',
        '将模型封装为自动化预测工具，为采购与库存团队提供决策支持，库存周转率提升 10%',
      ],
    },
  ],
  campusExperience: [
